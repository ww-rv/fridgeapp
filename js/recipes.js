...
function buildPrompt(category) {
  ...
  return `Ти — Євген Клопотенко, відомий український шеф-кухар. Твоя філософія: сміливі поєднання, переосмислення традицій, доступність, але з "вау-ефектом".

У мене є такі продукти: ${items}.
${urgent ? `Терміново потрібно використати: ${urgent}.` : ''}

ОБОВ'ЯЗКОВО ДОТРИМУЙСЯ БЕЗПЕКИ: Ніколи не поєднуй продукти, які можуть бути зіпсованими або несумісними в сирому вигляді. Якщо продукти з "термінових" не сумісні, краще запропонуй різні рецепти.

Запропонуй 2-3 рецепти категорії "${category}" з цих інгредієнтів. Опиши рецепти у твоєму фірмовому стилі, енергійно та креативно.
...
async function callAI(provider, prompt, b64Image = null, mimeType = 'image/jpeg') {
  const key = localStorage.getItem(`ai_key_${provider}`);
  if (!key) throw Object.assign(new Error('Немає ключа'), { type: 'no_key' });

  if (provider === 'claude') {
    const content = [{ type: 'text', text: prompt }];
    if (b64Image) content.push({ type: 'image', source: { type: 'base64', media_type: mimeType, data: b64Image } });
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: 'claude-3-5-haiku-20241022', max_tokens: 1500, messages: [{ role: 'user', content }] }),
    });
  ...
  if (provider === 'openai') {
    const content = [{ type: 'text', text: prompt }];
    if (b64Image) content.push({ type: 'image_url', image_url: { url: `data:${mimeType};base64,${b64Image}` } });
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${key}` },
      body: JSON.stringify({ model: 'gpt-4o-mini', max_tokens: 1500, messages: [{ role: 'user', content }] }),
    });
  ...
  if (provider === 'gemini') {
    ...
    if (b64Image) parts.push({ inline_data: { mime_type: mimeType, data: b64Image } });
