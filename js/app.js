...
function renderIcon(val) {
  if (!val) return '';
  if (val.endsWith('.svg')) return `<img src="Icon/${val}" class="p-icon" alt="">`;
  return `<span class="p-icon emoji">${val}</span>`;
}
function prodIcon(p)  { return renderIcon(p.icon || ICONS[p.cat] || 'icons8-grocery-100.svg'); }
...
document.addEventListener('click', e => {
  ...
  else if (btn.classList.contains('waste')) wasteCard(id);
...
function ph(p) {
  ...
  return `<div class="card-wrap">
    <div class="card-swipe-actions">
      ...
      <button class="swipe-act waste" data-id="${p.id}" style="color:#dc3545">🗑️<span>Зіпсувалось</span></button>
...
function renderStats() {
  ...
  <div class="bar-card">${catA.map(([k,v],i) => `<div class="brow">
    <div class="blbl"><div class="mini">${renderIcon(ICONS[k]||'icons8-grocery-100.svg')}</div><span>${CAT_NAME[k]||k}</span></div>
...
function renderShop() {
  ...
  if (active.length) {
    ...
    [...ordered.map(g => g.key), ...rest].forEach(cat => {
      ...
      const info = CAT_GROUPS.find(g => g.key === cat) || { label: 'Інше', icon: 'icons8-grocery-100.svg' };
...
function openMod() {
  ...
  currentIcon = 'icons8-grocery-100.svg'; currentIconCat = 'other';
  const prev = document.getElementById('icon-preview');
  prev.innerHTML = renderIcon('icons8-grocery-100.svg'); prev.style.background = 'var(--bg-sunken)'; prev.style.borderStyle = 'dashed';
  document.getElementById('ficon').value = 'icons8-grocery-100.svg';
...
function addProd() {
  ...
  const icon    = document.getElementById('ficon').value || 'icons8-grocery-100.svg';
