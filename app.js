...
function openQuickActions(id) {
  ...
  document.getElementById('quick-menu').innerHTML = `
    ...
    <div class="qa-grid">
      ...
      <button class="qa-btn danger" onclick="qaDelete()">${SVG.close}<span>Видали</span></button>
    </div>`;
  document.getElementById('quick-overlay').classList.add('on');
}
function closeQuickActions() { document.getElementById('quick-overlay').classList.remove('on'); qaTarget = null; }
...
function initSwipes() {
  document.querySelectorAll('.card[data-id]').forEach(card => {
    ...
    const move  = e => { if (!active) return; const t = e.touches?.[0] || e; dx = Math.min(0, Math.max(-162, t.clientX - sx)); card.style.transform = `translateX(${dx}px)`; };
    const end   = () => { if (!active) return; active = false; card.style.transition = 'transform .3s var(--ease)'; card.style.transform = dx < -80 ? 'translateX(-162px)' : 'translateX(0)'; };
...
function chQ(id, d) {
  const p = prods.find(x => x.id === id); if (!p) return;
  if (d > 0) { p.qty += d; p.used = (p.used||0)+1; }
  else {
    p.qty += d;
    if (p.qty <= 0) {
      p.wasted = (p.wasted||0)+1;
...
function _buildCalHistory() {
  ...
  const h = (a,b) => {
    let x = (a*1103515245 + b*12345 + 7919) >>> 0;
  ...
  for (let i = 0; i < DAYS; i++) {
    ...
    const items = [];
    all.forEach(p => {
      if (!p.kcal || !p.g) return;
      const r = h(p.id, i);
      const prob = Math.min(0.85, ((p.used||0) + 1) / 14);
      if (r < prob) {
        const frac = 0.25 + h(p.id, i*7+1) * 0.55;
        let portionG = (p.unit==='кг'||p.unit==='л') ? Math.round(p.g*.15*frac) : p.g>400 ? Math.round(p.g*.2*frac) : Math.round(p.g*frac);
        portionG = Math.max(20, portionG);
        items.push({ id:p.id, name:p.name, icon:prodIcon(p), cat:p.cat, g:portionG, kcal:Math.round(portionG*p.kcal/100) });
      }
    });
    out.push({ date:iso, items, total:items.reduce((s,x)=>s+x.kcal,0) });
  }
  return out.reverse();
}
...
function renderStats() {
  ...
  <div class="streak"><div class="flame">🔥</div><div style="flex:1"><div class="st">Серія без втрат</div><div class="sv">12 днів</div><div class="ssub">Нічого не пропало. Так тримати!</div></div></div>
  ...
  <div class="bar-card">${catA.map(([k,v],i) => `<div class="brow">
    <div class="blbl"><div class="mini">${ICONS[k]||'📦'}</div><span>${CAT_NAME[k]||k}</span></div>
...
function renderShop() {
  ...
  if (active.length) {
    ...
    [...ordered.map(g => g.key), ...rest].forEach(cat => {
      ...
      const info = CAT_GROUPS.find(g => g.key === cat) || { label: 'Інше', icon: '📦' };
      h += `<div class="shop-cat-header">${info.icon} ${info.label}</div>`;
...
function openMod() {
  ...
  prev.textContent = '📦'; prev.style.background = 'var(--bg-sunken)'; prev.style.borderStyle = 'dashed';
